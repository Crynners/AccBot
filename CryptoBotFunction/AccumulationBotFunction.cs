...
namespace CryptoBotFunction
{
    public class AccumulationBotFunction
    {
        ...
        public AccumulationBotFunction(ILoggerFactory loggerFactory)
        {
            _logger = loggerFactory.CreateLogger<AccumulationBotFunction>();
        }

        [Function("AccumulationBotFunction")]
        public async Task Run([TimerTrigger("%DayDividerSchedule%"
#if DEBUG || TEST
            , RunOnStartup = true
#endif
            )] TimerInfo myTimer)
        {
            ...
            var accumulationBot = new AccumulationBot(_logger);
            await accumulationBot.Tick();
        }
        ...
        private static void LoadAppSettings()
        {
            ...
            BotConfiguration.ChunkSize = Int32.Parse(config["ChunkSize"] ?? "0");
            ...
            BotConfiguration.MaxWithdrawalPercentageFee = isPercentageFeeValid ? MaxWithdrawalPercentageFee : 0.001m;
